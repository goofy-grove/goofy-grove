...
impl Default for FilesPoliciesConfig {
    fn default() -> Self {
        Self {
            user_avatar: FilePolicyConfig {
                max_file_size: default_max_file_size(),
                allowed_content_types: default_allowed_content_types(),
            },
            persona_avatar: FilePolicyConfig {
                max_file_size: default_max_file_size(),
                allowed_content_types: default_allowed_content_types(),
            },
        }
...
impl FilesPoliciesConfig {
    pub fn to_domain_policy(&self, scope: &FileScope) -> Option<FilePolicy> {
        ...
        let max_size = FileSize::try_new(config.max_file_size.to_bytes() as usize).ok()?;
        let allowed_content_types = config
            .allowed_content_types
            .iter()
            .filter_map(|value| FileContentType::try_new(value.clone()).ok())
            .collect();
